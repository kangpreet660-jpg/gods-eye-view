...
Produce a serial, multi-chapter narrative digest of a project's complete claude-mem history. Differs from `timeline-report` (one long report) — this generates one digest _per ISO week_, with each subagent reading the prior week's carry-forward block so the story stays coherent.
