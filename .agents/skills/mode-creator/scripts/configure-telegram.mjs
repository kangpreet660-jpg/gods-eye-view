...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 2) {
    ...
    if (!token?.startsWith('--') || !value)
      fail('usage: configure-telegram.mjs [--types <csv>] [--concepts <csv>]');
...
async function promptLine(label, { hidden = false, allowEmpty = false } = {}) {
  if (!process.stdin.isTTY || !process.stdout.isTTY) {
    fail(
      'interactive terminal required; run this helper directly in your terminal',
    );
  }
...
async function askYesNo(label, defaultYes = true) {
  ...
  const answer = (
    await promptLine(`${label}${suffix}`, { allowEmpty: true })
  ).toLowerCase();
...
try {
  ...
  if (!token && settings.CLAUDE_MEM_TELEGRAM_BOT_TOKEN) {
    if (
      await askYesNo('Use the Telegram bot token already saved in claude-mem?')
    ) {
      token = settings.CLAUDE_MEM_TELEGRAM_BOT_TOKEN;
    }
  }
  if (!token)
    token = await promptLine('Paste the BotFather token (input is hidden): ', {
      hidden: true,
    });
  ...
  if (!chatId && settings.CLAUDE_MEM_TELEGRAM_CHAT_ID) {
    if (
      await askYesNo(
        `Use the saved chat ID ${settings.CLAUDE_MEM_TELEGRAM_CHAT_ID}?`,
      )
    ) {
  ...
  if (!chatId) {
    console.log(
      `Open https://t.me/${bot.username}, press Start, and send any message.`,
    );
    await promptLine('Press Enter after the message has been sent: ', {
      allowEmpty: true,
    });
    try {
      ...
      if (
        chats.length === 1 &&
        (await askYesNo(`Use ${chats[0][1]} (${chats[0][0]})?`))
      ) {
        chatId = chats[0][0];
      ...
      console.warn(
        `Could not discover the chat automatically (${error instanceof Error ? error.message : String(error)}).`,
      );
      console.warn(
        'This commonly happens when the bot already has a webhook; enter the chat ID manually.',
      );
    }
  }
  if (!chatId)
    chatId = await promptLine(
      'Telegram chat ID (numeric, groups are usually negative): ',
    );
  if (!/^-?\d+$/.test(chatId) && !/^@[A-Za-z0-9_]{5,}$/.test(chatId))
    fail('chat ID must be numeric or an @channel username');
