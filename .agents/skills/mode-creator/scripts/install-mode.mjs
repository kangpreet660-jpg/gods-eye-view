...
function resolveDataDir() {
  ...
  return flat.CLAUDE_MEM_DATA_DIR
    ? expandHome(flat.CLAUDE_MEM_DATA_DIR)
    : defaultDir;
}
...
function validateItems(items, label, requiredFields) {
  ...
  for (const [index, item] of items.entries()) {
    ...
    for (const field of requiredFields)
      requireString(item[field], `${label}[${index}].${field}`);
    if (!ITEM_ID_PATTERN.test(item.id))
      fail(`${label}[${index}].id must be lowercase kebab-case`);
...
function validateMergedMode(mode) {
  ...
  validateItems(mode.observation_types, 'observation_types', [
    'id',
    'label',
    'description',
    'emoji',
  ...
  validateItems(mode.observation_concepts, 'observation_concepts', [
    'id',
    'label',
    'description',
  ]);
  if (
    !mode.prompts ||
    typeof mode.prompts !== 'object' ||
    Array.isArray(mode.prompts)
  ) {
  ...
  for (const prompt of REQUIRED_PROMPTS)
    requireString(
      mode.prompts[prompt],
      `prompts.${prompt}`,
      prompt === 'format_examples',
    );
  for (const type of mode.observation_types) {
    if (!mode.prompts.type_guidance.includes(type.id))
      fail(`prompts.type_guidance does not mention type: ${type.id}`);
  }
  for (const concept of mode.observation_concepts) {
    if (!mode.prompts.concept_guidance.includes(concept.id))
      fail(`prompts.concept_guidance does not mention concept: ${concept.id}`);
  }
}
...
if (!args.mode)
  fail(
    'usage: install-mode.mjs --mode <draft.json> [--mode-id <id>] [--telegram-types <csv>] [--telegram-concepts <csv>] [--dry-run]',
  );

const sourcePath = path.resolve(args.mode);
if (!existsSync(sourcePath)) fail(`draft mode file not found: ${sourcePath}`);
const modeId =
  args['mode-id'] ?? path.basename(sourcePath, path.extname(sourcePath));
...
if (existsSync(path.join(bundledModesDir, `${modeId}.json`))) {
  fail(
    `mode ID collides with bundled mode: ${modeId}; choose a unique custom ID instead of shadowing bundled configuration`,
  );
}
...
if (inheritanceParts.length === 2) {
  ...
  const parentPath = parentCandidates.find((candidate) =>
...
for (const type of telegramTypes)
  if (!typeIds.has(type))
    fail(`Telegram trigger type is not in this mode: ${type}`);
for (const concept of telegramConcepts)
  if (!conceptIds.has(concept))
    fail(`Telegram trigger concept is not in this mode: ${concept}`);
...
if (telegramTypes.length > 0 || telegramConcepts.length > 0) {
  ...
  settings.CLAUDE_MEM_TELEGRAM_TRIGGER_TYPES = mergeCsv(
    settings.CLAUDE_MEM_TELEGRAM_TRIGGER_TYPES,
    telegramTypes,
  );
  settings.CLAUDE_MEM_TELEGRAM_TRIGGER_CONCEPTS = mergeCsv(
    settings.CLAUDE_MEM_TELEGRAM_TRIGGER_CONCEPTS,
    telegramConcepts,
  );
}
