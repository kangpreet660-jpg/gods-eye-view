...
5. **Concurrency.** There is no shared log file to guard: each observation
   ...
   entry. Before changing an existing observation's _status_, re-read that
...
and only the recorded field makes the _absence_ of the judgement visible.
...
_enforcement_ is too weak, and nobody but the agent can see it. Log it in
...
skill:
  [skill-a, 'plugin:skill-b'] # existing skills this improves —
...
siblings_checked:
  'family-name: a, b — a added; b excluded (b/SKILL.md §2)'
...
review — "directly" means _now_, not _in place_: the edit is still made on
