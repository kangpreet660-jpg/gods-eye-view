...
still _extracts_ — a regex that slices between the two `---` lines is happy —
...
So the test is not only _can this condition occur at all_ but **can the
...
to adjudicate: the whole point of the trace is to show that _this_ session
...
**Four states, one empty result.** An instrument that _ran and found
nothing_, one that _ran and broke_, one that _was never permitted to
run_ and one that _has not finished_ (below) all return the same thing,
...
practice, as being about _those_. It is not, and the gap is precisely where
...
producer still running that has found nothing _yet_ is byte-identical to
...
write asks only for _facts about_ the files: how many exist, how many have a
...
The general form belongs with the instrument guard below: _the instrument
was not permitted to run_ is a third state beside _it ran and found nothing_
and _it ran and broke_. All three produce an empty result, and only the
middle one is a fact about the world.

### A refused snippet is a degraded path, never a skipped step

The refusal above is of the print alone; a harness that vets command
_shape_ can refuse the whole block. A policy layer that classifies
...
itself generic _inside one artefact_ — "this applies to any file-writing
...
expressible. The mechanism exists; the _check_ does not — and a list field
...
check generates noise instead of signal. Record the _coherence model_ for
...
makes its _absence_ visible, which is the only property that lets anything
...
the absence of the _ritual_ visible; it does nothing for the absence of
the _grounds_, and once the ritual is habitual the failure moves from the
...
concurrency story" below): two sessions that pick the same id _and_ the
...
the same number with _different_ slugs produce two different paths, both
...
_relevance_ queries; the id is a _maximum_, and the answer to one is never
...
above cannot stop two sessions legitimately resolving the _same_ file in
...
observation is an _untracked_ file until someone commits it, and
...
1. _A session can contain no todo items whatsoever._ The 3rd-completion
...
2. _"Is this a major deliverable?" is a self-assessment, and self-assessment is
   what fails under load._ Prefer triggers unmistakable in the tool record over
