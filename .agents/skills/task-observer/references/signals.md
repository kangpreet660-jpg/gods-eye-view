...
deciding when shared modules should be centralised". Knowing when _not_
...
| 2nd        | rule broken _again_, same rule      | wording, more emphatic                |
...
executing rather than announcing does not catch it. So before writing _any_
...
adds nothing (act now). A criterion you _can_ name must also be able to occur:
