...
running something _equivalent_ verifies your paraphrase, not the
...
_use_ without checking — and a URL, an API endpoint, a database name, a file
...
_something_ — an empty result set, a login page, a shell — and the natural
...
1. **The skill** holds the process, and _reads_ configuration.
...
State the precedence explicitly: config wins for _configuration_, the
skill wins for _process_, and config can never relax a rule. Give the
...
7. **Cross-skill reference scan (published skills)** — a skill written
   ...
   dependency on a specific _published_ open-source companion. Scan for
...
1. The live file is the authoritative source: in Claude Code,
   ...
   **That guard exists in Cowork only.** In Claude Code and most
   ...
   on Observations") decides _when_ a small change is made — now, rather
   than at the next review — never _where_; it does not license an
   in-place edit.
...
9. **Inserting into an ordered structure is a splice, not a replacement.**
   ...
   because an _unchanged_ line had moved relative to its neighbours and
...
Observed, in one session: the loader served a _previous lineage_ of a
...
question is what a _future_ session will receive, only a genuinely new
...
resolved before any work. The user's correction was one line: _don't ask
about the institution — just ask how many items I want of each kind._ They
...
sentence goes (_could this sentence survive having the platform's name
removed?_ → core; otherwise → companion), the confidentiality rules, the
...
prerequisites as _checkable facts_, so that something other than a human
...
ones that resolve _and cannot run_. A dead target and a deleted target both
