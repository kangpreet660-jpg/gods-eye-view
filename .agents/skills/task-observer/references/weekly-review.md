...
- **Scope by what the logs observe, not by what they declare.** Logs
  ...
  entries are routinely filed against _different_ skills, so a
...
- **Stage each affected skill ONCE, and publish the anchor workspace
  ...
  **anchor directory** (`[today]/[skill-name]`, or a discriminated one
  ...
  senses are separate axes and are always named in full: _anchor
  workspace_ = which log's `skill-updates/` tree, _anchor directory_ =
  which dated directory inside it. Append the manifest entry in the
  anchor workspace AND a pointer entry in every _other_ participating
...
**Classify before you ask.** Any _disposition_ option offered to the user
...
_previous_ sessions — with the sweep as shipped (the sweep block of the id
...
copy, ask whether live _supersedes_ it — carries a line that contains
it, or evidently replaces it — or _lacks_ it: every such line
...
classifies every change on it as _exercised naturally by a week of use_
or _unlikely to happen naturally_, and for the second class writes a test
...
below is defined against a staged copy of the _live_ file, so it is
...
live path _inside_ the staged directory, once per skill. And keep `IFS=` on
...
4. **Characterisations, not just values** — the verification pass reads
   ...
   and a wrong characterisation is consumed by being _agreed with_,
...
catch are the ones created _during_ the run (observed: Step 1 found and
...
queued handover — its _measured_ values are usually filled in correctly at
