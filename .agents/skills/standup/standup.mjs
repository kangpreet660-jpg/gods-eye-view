...
function parseWindowMs(s) {
  ...
  if (!m) {
    console.error(
      `standup: unrecognized window "${s}" — showing all worktrees`,
    );
    return null;
  }
...
async function cmdPRs() {
  ...
  if (prs == null)
    die(
      'gh unavailable, unauthenticated, or no GitHub remote (try `gh auth login`)',
    );
  ...
  if (windowMs != null)
    rows = rows.filter((p) => p.updatedMs && now - p.updatedMs <= windowMs);
  ...
  if (!rows.length) {
    console.error(
      windowMs != null
        ? `no open PRs updated within "${opts.since}"`
        : `no open PRs`,
    );
    return;
  }
  for (const p of rows) {
    ...
    console.log(
      `#${p.number}\t${p.age.padEnd(8)}\t${p.branch}\t${p.title}${draft}`,
    );
  }
}
...
async function cmdWorktrees() {
  ...
  if (windowMs != null) {
    rows = rows.filter(
      (w) => w.lastActivityMs && now - w.lastActivityMs <= windowMs,
    );
  }
...
async function cmdOpen() {
  ...
  if (await exists(FILE)) {
    if (!opts.force)
      die(`channel exists: ${FILE} (use --force to rotate it aside)`);
    const archived = FILE.replace(
      /\.md$/,
      `-${nowIso().replace(/[:T]/g, '-').replace('Z', '')}.md`,
    );
...
async function cmdWatch() {
  ...
  for (;;) {
    ...
    console.log(
      `NEW (${others.length}) after ${Math.round((Date.now() - start) / 1000)}s:\n`,
    );
